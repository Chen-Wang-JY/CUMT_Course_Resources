...
int	add_symbol (short t, char* token) 
{
	...
	length  = strlen(token);
	p = (char*) malloc( (length+1)*sizeof(char) );  //?????1???????, ??????
	if(p == NULL){
	...
	if (sti < 0) 
	{
		...
		hashvec[cell] = n_symbols;					// Put symbol number into hash vector.     //hash table?§Õ›¥??????????cell????????????????
		...
		symbol[n_symbols].cell = cell;			// Define hash vector cell number for this symbol.  //?????cell???????hash???§Ö??????¡À????????
...
int   get_symbol (char* token) 
{
	...
	length  = strlen(token);  	// Set length. //????
...
Cont:    cell = (hash *= 65549)/hashdiv;	// Get new cell number.
		...
		while (i >= 0);      					// While not empty slot.  //???i<0,?????????????cell??????????¦Ë??(?????¡À?)
	}
...
void adjust_term( Node * node1, int term )
{
	...
	if(strcmp(node_name[node[m].id], "typedef_") == 0){  //typedef_  // && node[m].bsource ??????????????????
		r = node[m].next;
