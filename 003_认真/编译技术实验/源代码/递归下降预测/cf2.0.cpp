...
DFA::DFA(){
    ...
    {"if", "then", "else", "end", "repeat", "until", "read", "write", "int", "double",
    "float", "bool", "while", "include", "using", "namespace", "std", "iostream", "return",
    "cin", "cout", "endl", "string", "static", "do", "break"};
    _programPoint = 0;
    _token = "";
    _row = 1;
    _nextRow = false;
...
void DFA::start(){
    ...
    while(_programPoint < _program.size() - 1){
        ...
        //出错
        if(sym < 0){
            cout << endl << "词法分析出错！错误位于第[" << _row << "]行。" << endl;
            errorHandling(sym);
            break;
        }
        else{
